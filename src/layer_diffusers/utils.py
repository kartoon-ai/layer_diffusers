...
def save_latent(func, save_to):
    def wrapper(*args, **kwargs):
        save_to.append(args[0])
        return func(*args, **kwargs)

    return wrapper


def tensor_to_numpy_img(t, normalize=False):
    if normalize:
        t = (t - t.min()) / (t.max() - t.mean())
    return (t * 255.0).detach().cpu().float().numpy().clip(0, 255).astype(np.uint8)


def latents_to_img(pipeline, latents):
    image = pipeline.vae.decode(latents).sample.detach().cpu()

    # from [-1, 1] to [0, 1]
    image = (image / 2 + 0.5).clamp(0, 1)
    # return is a torch tensor
    return image
...
def generate_img_with_checkerboard(fg, alpha):
    H, W, C = fg.shape
    cb = checkerboard(shape=(H // 64, W // 64))
    cb = cv2.resize(cb, (W, H), interpolation=cv2.INTER_NEAREST)
    cb = (0.5 + (cb - 0.5) * 0.1)[None, ..., None]
    cb = torch.from_numpy(cb).to(fg)
    return fg * alpha + cb * (1 - alpha)
